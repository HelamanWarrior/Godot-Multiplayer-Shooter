...
=======
	if (is_network_master()):  # se esta ejecutando esta instancia en el cliente que controla el nodo.
		if (playerSeeking):
			var posPlayer = (playerSeeking.global_position - position).normalized()
			vectorPosicion = move_and_slide(posPlayer * speed)
			facing = look_at(posPlayer)
		rset_unreliable('puppet_vectorPosicion', vectorPosicion)
		...
		vectorPosicion = puppet_vectorPosicion
